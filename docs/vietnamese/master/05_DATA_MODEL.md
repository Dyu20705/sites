...
**PROPOSED — D05/D09.** Các khái niệm dưới đây là vocabulary cho research và design. Chúng **không phải** class, table hay SQL schema đã được chấp thuận.
...
| TemporalMetric | Giá trị đo cùng unit, interval, timestamp semantics và missingness | Phải phân biệt metric do nguồn cung cấp với metric hệ thống tính |
...
| Ingestion | SITES nạp dữ liệu khi nào? | Không phản ánh chronology của research |
...
Event date trước cutoff **không đủ** để chứng minh field đã available tại cutoff. Dataset tải hôm nay có thể chỉ hỗ trợ retrospective analysis dù record có ngày lịch sử.
...
Đề xuất hiện tại là ưu tiên source-local identity trong một corpus để giảm scope, nhưng vẫn phải test duplicate và revision.
